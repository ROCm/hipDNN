...
TEST(ConvolutionFwdAttributesTests, SetAndGetConvParameters3D)
{
    ...
    std::vector<int64_t> post_padding_3d = {1, 2, 3};
    std::vector<int64_t> stride_3d = {1, 2, 3};
    std::vector<int64_t> dilation_3d = {1, 1, 1};

    conv_attributes.set_pre_padding(pre_padding_3d);
    conv_attributes.set_post_padding(post_padding_3d);
    conv_attributes.set_stride(stride_3d);
    conv_attributes.set_dilation(dilation_3d);

    EXPECT_EQ(conv_attributes.get_pre_padding(), pre_padding_3d);
    EXPECT_EQ(conv_attributes.get_post_padding(), post_padding_3d);
    EXPECT_EQ(conv_attributes.get_stride(), stride_3d);
    EXPECT_EQ(conv_attributes.get_dilation(), dilation_3d);
}
