...
TEST(DBNNodeTests, PackNode)
{
    ...
    EXPECT_EQ(packed_attributes->dy_tid(), dy_tensor->get_uid());
    ...
    EXPECT_EQ(packed_attributes->dx_tid(), dx_tensor->get_uid());
    EXPECT_EQ(packed_attributes->dscale_tid(), dscale_tensor->get_uid());
    EXPECT_EQ(packed_attributes->dbias_tid(), dbias_tensor->get_uid());
}
...
TEST(DBNNodeTests, PackNodeWithoutMeanAndInvVariance)
{
    ...
    EXPECT_EQ(packed_attributes->x_tid(), x_tensor->get_uid());
    EXPECT_EQ(packed_attributes->scale_tid(), scale_tensor->get_uid());
    EXPECT_EQ(packed_attributes->mean_tid(), flatbuffers::nullopt);
    EXPECT_EQ(packed_attributes->inv_variance_tid(), flatbuffers::nullopt);
    EXPECT_EQ(packed_attributes->dx_tid(), dx_tensor->get_uid());
    EXPECT_EQ(packed_attributes->dscale_tid(), dscale_tensor->get_uid());
    EXPECT_EQ(packed_attributes->dbias_tid(), dbias_tensor->get_uid());
}
