...
void Engine_plugin::resolve_symbols()
{
    ...
    _func_destroy_handle = _lib.get_symbol<decltype(_func_destroy_handle)>(func_name_destroy_handle);
    ...
    _func_get_applicable_engine_ids = _lib.get_symbol<decltype(_func_get_applicable_engine_ids)>(func_name_get_applicable_engine_ids);

    const auto func_name_get_engine_details = "hipdnnEnginePluginGetEngineDetails";
    _func_get_engine_details = _lib.get_symbol<decltype(_func_get_engine_details)>(func_name_get_engine_details);

    const auto func_name_destroy_engine_details = "hipdnnEnginePluginDestroyEngineDetails";
    _func_destroy_engine_details = _lib.get_symbol<decltype(_func_destroy_engine_details)>(func_name_destroy_engine_details);

    const auto func_name_get_workspace_size = "hipdnnEnginePluginGetWorkspaceSize";
    _func_get_workspace_size = _lib.get_symbol<decltype(_func_get_workspace_size)>(func_name_get_workspace_size);

    const auto func_name_create_execution_context = "hipdnnEnginePluginCreateExecutionContext";
    _func_create_execution_context = _lib.get_symbol<decltype(_func_create_execution_context)>(func_name_create_execution_context);

    const auto func_name_destroy_execution_context = "hipdnnEnginePluginDestroyExecutionContext";
    _func_destroy_execution_context = _lib.get_symbol<decltype(_func_destroy_execution_context)>(func_name_destroy_execution_context);

    const auto func_name_execute_op_graph = "hipdnnEnginePluginExecuteOpGraph";
    _func_execute_op_graph = _lib.get_symbol<decltype(_func_execute_op_graph)>(func_name_execute_op_graph);
...
std::vector<int64_t> Engine_plugin::get_applicable_engine_ids(hipdnnEnginePluginHandle_t handle, const hipdnnPluginConstData_t* op_graph) const
{
    assert(_initialized);
    ...
    auto status = _func_get_applicable_engine_ids(handle, op_graph, engine_ids.data(), max_engines, &num_engines);
    if (status != HIPDNN_PLUGIN_STATUS_SUCCESS)
    {
        throw Hipdnn_exception(HIPDNN_STATUS_PLUGIN_ERROR,
    ...
    if (num_engines > max_engines)
    {
        ...
        status = _func_get_applicable_engine_ids(handle, op_graph, engine_ids.data(), max_engines, &num_engines);
        if (status != HIPDNN_PLUGIN_STATUS_SUCCESS)
        {
            throw Hipdnn_exception(HIPDNN_STATUS_PLUGIN_ERROR,
...
hipdnnEnginePluginExecutionContext_t Engine_plugin::create_execution_context(hipdnnEnginePluginHandle_t handle,
                                                                             const hipdnnPluginConstData_t* engine_config,
                                                                             const hipdnnPluginConstData_t* op_graph) const
{
    assert(_initialized);
...
void Engine_plugin::destroy_execution_context(hipdnnEnginePluginHandle_t handle,
...
{
    assert(_initialized);
    auto status = _func_execute_op_graph(handle, execution_context, workspace, device_buffers, num_device_buffers);
    if(status != HIPDNN_PLUGIN_STATUS_SUCCESS)
    {
        throw Hipdnn_exception(HIPDNN_STATUS_PLUGIN_ERROR,
                               "Failed to execute op graph. Status code: "
                                   + std::to_string(status)
                                   + ", Error: " + std::string(get_last_error_string()));
    }
}
