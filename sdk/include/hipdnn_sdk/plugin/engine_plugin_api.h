 ...
 *
 * @param[out] handle A pointer to a variable where the created engine plugin handle will be stored.
 *                    The handle is used for subsequent operations on the engine plugin.
 *
 * @return A value of type `hipdnnPluginStatus_t` indicating the status of the operation.
 *
 * @note The caller is responsible for ensuring that the handle is destroyed properly
 ...
 * @return A value of type `hipdnnPluginStatus_t` indicating the status of the operation.
 *
 * @note The caller must ensure that the provided stream remains valid for the duration
 *       of operations performed using the engine plugin handle.
 */
HIPDNN_PLUGIN_EXPORT hipdnnPluginStatus_t
 ...
 * @param[out] engine_ids A pointer to an array where the IDs of the applicable engines will be stored.
 ...
 *                         This value may exceed `max_engines` if more engines are applicable than the array can accommodate.
 *
 * @return A value of type `hipdnnPluginStatus_t` indicating the status of the operation.
 *
 ...
 * @param[in] op_graph A pointer to a structure where the serialized `Graph` from `graph.fbs` is stored.
 * @param[in,out] engine_details A pointer to a structure where the serialized `EngineDetails` from `engine_details.fbs`
 *                               will be stored.
 *
 * @return A value of type `hipdnnPluginStatus_t` indicating the status of the operation.
 *
 ...
 *
 * @param[in] handle The engine plugin handle.
 * @param[in] engine_config A pointer to a structure where the serialized `EngineConfig` from `engine_config.fbs` is stored.
 * @param[in] op_graph A pointer to a structure where the serialized `Graph` from `graph.fbs` is stored.
 * @param[out] workspace_size A pointer to a variable where the required workspace size (in bytes)
 *                            will be stored.
 *
 * @return A value of type `hipdnnPluginStatus_t` indicating the status of the operation.
 */
 ...
 * @param[in] engine_config A pointer to a structure where the serialized `EngineConfig` from `engine_config.fbs` is stored.
 * @param[in] op_graph A pointer to a structure where the serialized `Graph` from `graph.fbs` is stored.
 * @param[out] execution_context A pointer to a variable where the created execution context will
 *                               be stored.
 *
 * @return A value of type `hipdnnPluginStatus_t` indicating the status of the operation.
 *
 * @note After calling this function, the engine configuration and the operation graph are no longer needed, 
 ...
 *                               and the engine configuration to be executed.
 * @param[in] workspace A pointer to the workspace memory allocated by the user. The workspace
