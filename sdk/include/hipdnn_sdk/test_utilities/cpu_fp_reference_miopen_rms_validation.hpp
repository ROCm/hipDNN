...
public:
    ...
    {
        if(relative_tolerance < static_cast<T>(0.0f))
        {
                 ...
                 Migratable_memory_interface<T>& implementation) override
    {
        ...
        // Calculate and compare to _relative_tolerance
        return (std::sqrt(square_difference) / (std::sqrt(static_cast<double>(element_count)) * max_magnitude)) < static_cast<double>(_relative_tolerance);
    }
