...
void compute_cpuref_maxpool_fwd(test_2dpool_desc_t pd, dataType *src,
                                dataType *dst) {
      ...
      for (int oh = 0; oh < pd.oh; oh++) {
        for (int ow = 0; ow < pd.ow; ow++) {
          size_t oidx = (size_t)n * pd.c * pd.oh * pd.ow +
                        (size_t)c * pd.oh * pd.ow + (size_t)oh * pd.ow + ow;
          ...
          for (int kh = 0; kh < pd.kh; ++kh)
            for (int kw = 0; kw < pd.kw; ++kw) {
              ...
              size_t iidx = (size_t)n * pd.c * pd.ih * pd.iw +
                            (size_t)c * pd.ih * pd.iw + (size_t)ih * pd.iw + iw;
