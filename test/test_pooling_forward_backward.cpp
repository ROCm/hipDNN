...
hipdnnPoolingMode_t poolI_mode;
...
TEST(pooling_fwd_back, func_check_fwd_bwd) {
  ...
  hipdnn_pooling_forward<float>(pool, srcData.gpu(), dstData.gpu(), poolI_mode,
                                &avg_time1);
  hipdnn_pooling_backward<float>(test_case, srcData.gpu(), gradData.gpu(),
                                 dstData.gpu(), poolI_mode, &avg_time2);
