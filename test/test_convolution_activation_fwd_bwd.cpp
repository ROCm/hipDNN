...
TEST(convolution_activation_fwd_bwd_intg, func_check_naive_conv_activation_fwd_bwd) {
  ...
  int ip_size_af[4] = {test_case.n, test_case.channels, test_case.height, test_case.width};
  int k_size_af[4] = {0,0,0,0};
  int op_size_af[4] =  {test_case.n, test_case.channels, test_case.height, test_case.width};

  int ip_size_ab[4] = {test_case.n, test_case.channels, test_case.height, test_case.width};
  int k_size_ab[4] = {0,0,0,0};
  int op_size_ab[4] =  {test_case.n, test_case.channels, test_case.height, test_case.width};
 ...
 std::string str_ip_size = integration_dims_to_string2(ip_size_cf,ip_size_af,ip_size_cb,ip_size_ab,"Conv_fwd","Act_fwd","Conv_bwd","Act_bwd");
 std::string str_k_size = integration_dims_to_string2(k_size_cf,k_size_af,k_size_cb,k_size_ab,"Conv_fwd","Act_fwd","Conv_bwd","Act_bwd");
 std::string str_op_size = integration_dims_to_string2(op_size_cf,op_size_af,op_size_cb,op_size_ab,"Conv_fwd","Act_fwd","Conv_bwd","Act_bwd");
  ...
  compute_hipdnn_activation_bwd<float>(test_case, dstDataGPU.gpu(), data_grad.gpu(), dataDst.gpu(), &avg_time3);
 compute_conv_back_filter<float>(testConvolutionSizes, srcDataConv.gpu(), filterData.gpu(), gradData2.gpu(), NULL, data_grad.gpu(), &avg_time4);
