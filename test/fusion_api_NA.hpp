...
void compute_hipdnn_fusion_api_NA(convulution_Size &c, dataType *src,
  ...
  hipdnnBatchNormMode_t BN_mode= HIPDNN_BATCHNORM_SPATIAL_PERSISTENT;
  ...
  hipMalloc(&resultSaveMean, 1 * c.oc *  sizeof(float));

  float *resultSaveInvVariance;
  hipMalloc(&resultSaveInvVariance, 1 * c.oc * sizeof(float));
