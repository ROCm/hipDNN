...
struct activation_params_t{
      ...
      activation_params_t(int n, int channels, int height, int width): n(n), channels(channels), height(height), width(width){}
};

template<typename dataType>
void compute_hipdnn_activation_backward(activation_params_t& test_case, dataType* src, dataType* grad, dataType* dst){
  ...
  checkHIPDNN(hipdnnSetTensor4dDescriptor(
        ...
        test_case.n, test_case.channels, test_case.height, test_case.width));
