...
void compute_hipdnn_fusion_api(convulution_Size &c, dataType *src,
  ...
  checkHIPDNN(hipdnnSetConvolution2dDescriptor(conv_desc, c.padh, c.padw,
                                               c.strh, c.strw, c.dilh, c.dilw,
                                               HIPDNN_CROSS_CORRELATION,
                                               HIPDNN_DATA_FLOAT));

  checkHIPDNN(hipdnnGetConvolution2dForwardOutputDim(conv_desc, in_desc,
                                                     filt_desc, &c.mb, &c.oc,
                                                     &c.oh, &c.ow));
  ...
  checkHIPDNN(hipdnnSetTensor4dDescriptor(bias_desc, HIPDNN_TENSOR_NCHW,
                                          HIPDNN_DATA_FLOAT, c.mb, c.oc,
                                          c.oh, c.ow));
  ...
  checkHIPDNN(hipdnnCreateOpConvForward( fusePlanDesc, &convOp, conv_desc,
                                         filt_desc));
