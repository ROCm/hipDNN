...
TEST(LRN_bwd, func_check_naive_LRN_backward) {
   Desc inputDesc(1, 3, 7, 7);
    ...
    LRN_params_t LRN_params(
        inputDesc.N, inputDesc.C, inputDesc.H, inputDesc.W);
  ...
  compute_hipdnn_LRN_backward<float>(LRN_params, srcData.gpu(), gradDataGPU.gpu(), dstDataGPU.gpu(), &avg_time);
   ...
   std::cout << "\nAverage Time is: " << avg_time << "micro seconds"<<std::endl;
    ...
    std::string str  = convert_to_string((float*)temp,(int)gradDataGPU.get_num_elements());
    //std::cout<<"################### LRN BWD OUTPUT ##########################"<<str;
