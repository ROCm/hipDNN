...
void Engine_manager::add_engine(std::unique_ptr<EngineInterface> engine)
{
    ...
    Engine_manager::get_applicable_engine_ids(const hipdnn_plugin::Graph_interface& op_graph)
{
    ...
    {
        if(engine.second->isApplicable(op_graph))
        {
                                        ...
                                        hipdnnPluginConstData_t& engine_details_out)
{
    ...
    engine.getDetails(handle, engine_details_out);
}
...
{
    auto& engine = get_engine(engine_id);
    return engine.getWorkspaceSize(handle, op_graph);
}
    ...
    hipdnnEnginePluginExecutionContext& execution_context) const
{
    ...
    engine.initializeExecutionContext(handle, op_graph, execution_context);
}

EngineInterface& Engine_manager::get_engine(int64_t engine_id) const
{
