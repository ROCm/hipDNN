...
void copyWithCheck(const flatbuffers::Vector<int64_t>* src, std::vector<int>& dst, size_t expectedSize, const char* name, const char* expectedSizeName)
{
    if(src != nullptr)
    {
        if(src->size() != expectedSize)
        {
            throw hipdnn_plugin::HipdnnPluginException(HIPDNN_PLUGIN_STATUS_BAD_PARAM, "MiopenConvDescriptor: " + std::string(name) + " size must be equal to " + std::string(expectedSizeName));
        }
        if(!std::ranges::all_of(*src, [](int64_t v) { return v <= std::numeric_limits<int>::max(); }))
        {
            throw hipdnn_plugin::HipdnnPluginException(HIPDNN_PLUGIN_STATUS_BAD_PARAM, "MiopenConvDescriptor: " + std::string(name) + " values must be less than INT_MAX");
        }
...
MiopenConvDescriptor::MiopenConvDescriptor(size_t spatialDimCount, const hipdnn_sdk::data_objects::ConvolutionFwdAttributes& attributes)
{
    if(spatialDimCount >= std::numeric_limits<int>::max())
    {
        throw hipdnn_plugin::HipdnnPluginException(HIPDNN_PLUGIN_STATUS_BAD_PARAM, "MiopenConvDescriptor: spatialDimCount must be less than INT_MAX");
    }

    const auto convMode = attributes.conv_mode();
    if(convMode != hipdnn_sdk::data_objects::ConvMode::ConvMode_CROSS_CORRELATION)
    {
        throw hipdnn_plugin::HipdnnPluginException(HIPDNN_PLUGIN_STATUS_BAD_PARAM, "MiopenConvDescriptor: only ConvMode_CROSS_CORRELATION is supported");
    }
    ...
    THROW_ON_MIOPEN_FAILURE(miopenInitConvolutionNdDescriptor(_descriptor, static_cast<int>(spatialDimCount), padding.data(), stride.data(), dilation.data(), miopenConvolution));
}
