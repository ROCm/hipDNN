...
hipdnnPluginDeviceBuffer_t
    generate_random_device_buffer(Tensor<T>& tensor, int uid, T min, T max, unsigned int seed = 0)
{
    ...
    const Tensor_layout& layout)
{
    ...
    Tensor<Input_type> x_tensor(dims, layout);
    ...
    Tensor<Input_type> y_tensor(dims, layout);
    device_buffers.push_back(generate_random_device_buffer(
        y_tensor, 2, static_cast<Input_type>(-100.0f), static_cast<Input_type>(100.0f), seed));

    Tensor<Intermediate_type> scale_tensor(derived_dims);
    ...
    Tensor<Intermediate_type> bias_tensor(derived_dims);
    ...
    Tensor<Intermediate_type> mean_tensor(derived_dims);
    ...
    Tensor<Intermediate_type> variance_tensor(derived_dims);
